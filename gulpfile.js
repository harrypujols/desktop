...
gulp.task("assets", function () {
  return gulp.src("./dev/assets/**/*").pipe(gulp.dest("./build/"));
});

gulp.task("media", function () {
  return gulp.src("./content/media/**/*").pipe(gulp.dest("./build/media"));
});
  ...
  if (site.posts) {
    ...
    site.posts.forEach(function (post) {
      ...
      if (c == count) {
        ...
        console.log(
          "page=" + page + " c=" + c + " posts.length=" + site.posts.length
        );
        file.page = {
          posts: posts,
          prev:
            page != 0
              ? basename + (page - 1 == 0 ? "" : page - 1) + ".html"
              : null,
          next:
            (page + 1) * count < site.posts.length
    ...
    if (posts.length != 0) {
      ...
      file.page = {
        posts: posts,
        prev:
          page != 0 ? basename + (page - 1 == 0 ? "" : page) + ".html" : null,
        next: null,
      };
      stream.write(file);
    }
...
gulp.task("cname", function () {
  return gulp.src("./dev/CNAME").pipe(gulp.dest("./build/"));
});
...
gulp.task("watch", function () {
  ...
  gulp.watch(
    [
      "./dev/templates/post.html",
      "./dev/templates/index.html",
      "./dev/templates/updates.html",
      "./content/posts/*.md",
    ],
    ["build"]
  );
